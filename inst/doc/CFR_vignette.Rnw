...
dat <- cbind(dat, new.times=NA)
dat[c(idx.for.Estep, idx.for.Estep+60),"new.times"] <- rep(new.times, 2)
@
...
alpha.start.values=alpha.start, full.data=dat, verb=FALSE,
